...
1. 调 `analyze_corpus_coverage(text)`：拿到文本汉字总数(totalHan)、不同字数(distinctHan)、
   被库覆盖的不同字数(coveredChars)、覆盖率(coveragePct)、高风险字数(highRiskHits)、高频覆盖字
   (topHits) 与未收录的库外字(uncovered)。这一步同时会在"形声流变·语料覆盖率"面板展示结果。
2. 从覆盖到的字里挑出**高 OCR 风险**与**多对一合并**的字（结合 topHits 与常识），对其中最关键的
...
4. 产出**识读风险报告**（Markdown）：
   - **概览**：总字数 / 不同字 / 库内覆盖字数 / 覆盖率% / 高风险字数（并点名几个库外字）
