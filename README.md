...
| 貳 · **古籍识读** | 古籍影像 OCR + 上下文校对 | ① OCR | 上传刻本/写本影像，调用**以古籍微调的 PaddleOCR-VL** 模型转写为文本，返回字数与推理延迟；单卡串行队列、可查看排队深度。识读后可一键**上下文校对**：结合文义与部件形近字表标出可疑识读、按把握度分级框标并给候选字，**只建议不改字**，关键存疑交专家定夺 |
...
POST /api/ocr/proofread      # 古籍文本上下文校对：标注可疑字 + 候选（不改原文）
...
### OCR 上下文校对（可疑字框标注 + 候选建议）

OCR 把"图"变成"文"，但形近误识难免。识读页的「校对」按钮对识读文本做一遍**上下文校对**，
定位为**辅助专家、而非自动改字**——可疑字框出来、给候选，判断权交给人。三层信号叠加：

- **OCR 逐字置信度（主闸门）**：本地 PaddleOCR-VL 解码时每个字的把握（softmax 概率）经 token→字对齐取出，
  低置信字优先送校对、其 top-k 次优读法直接当候选；OCR 自己没把握、上下文又没抓到的字单独标成"低置信"；
- **形近字先验**：从 `cl_analysis` 的 `ocr_confusion`（CHISE 部件结构算出的形近字对）建索引，作候选先验，
  也保证模型漏掉的形近孪生字仍出现在候选里；
- **上下文判定**：DeepSeek（复用史脉那套 `LLM_*`）结合文义、固定搭配、人名地名书名判断某字是否可疑。

每个风险字带两个独立刻度：`confidence`（语言模型判误识的把握）与 `ocr_confidence`（OCR 识别置信度）；
按等级分级框标，点击弹出候选 + 理由，专家采纳某候选才在本地替换、或"保留原字"。识读页还有「OCR 置信度」开关，
直接把整段按 OCR 逐字把握上色（越红越没把握）。位置由后端用原文片段反查、前后端一律按码点对齐，避免错位。

接口 `POST /api/ocr/proofread`、助手工具 `proofread_ocr`。置信度仅本地 PaddleOCR-VL 后端提供，
remote / vision 后端退化为"形近 + 上下文"两层。
